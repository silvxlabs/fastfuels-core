...
from fastfuels_core.crown_profiles import get_purves_radius, get_beta_radius, add_crown_profile_params
...
class DiscSampler:
    ...
    def sample(self, 
        ...
        if max_dist is None:
            ...
            tree_scale = np.linspace(1., 0., iterations)**2
            # Height error per iteration 
            height_error = np.linspace(0.0, 5., iterations)
        ...
        tree_grid, trees_to_place = sample_trees(chm, z_score, tree_grid, tree_coords, tree_props, dx, tree_scale, max_dist, height_error) 
...
@njit
def intersects(distance : float, props0 : NDArray, props1 : NDArray, scale=1., crown_profile_model='purves'):
    """
...
@njit 
def detect_intersection_purves(distance : float, props0 : NDArray, props1 : NDArray, scale=1.):  
    """
    ...
    r0_max = scale*get_purves_radius(cb_max, height0, crown_base0, max_radius0, shape0)
    r1_max = scale*get_purves_radius(cb_max, height1, crown_base1, max_radius1, shape1)

    return r0_max + r1_max >= distance


@njit 
def detect_intersection_beta(distance : float, props0 : NDArray, props1 : NDArray, scale=1.):
    """
...
def accept(
    ...
    valid = chm[i,j] >= 1. and tree_grid[i,j] == 0
    if not valid:
        return valid
    ...
    for i1 in range(max(1, i-w), min(n0-1, i+w)):
        for j1 in range(max(1, j-w), min(n1-1, j+w)):
            if tree_grid[i1,j1] > 0:
                ...
                collision = intersects(dist, props0, props1, tree_scale, crown_profile_model=crown_profile_model)
                if collision:
                    return False
...
def sample_trees(
    ...
    for i in range(len(tree_scale)):
        ...
        for j in range(len(indexes)):
            ...
            valid = accept(chm, z_score, tree_grid, tree_props, tree_index, xi, xj, dx, dx, tree_scale[i], height_error[i])
    ...
    for j in range(len(indexes)):
        ...
        xi, xj = local_search(tree_grid, xi, xj, dx=dx, max_dist=30.)
        tree_grid[xi, xj] = tree_index + 1
