...
def add_crown_profile_params(trees: TreePopulation, crown_profile_model='purves') -> TreePopulation:
    ...
    spcd_path = importlib.resources.files('fastfuels_core.data').joinpath('spcd_parameters.json')
    spcd_params = json.loads(spcd_path.read_text())
    
    spgrp_path = importlib.resources.files('fastfuels_core.data').joinpath('spgrp_parameters.json')
    ...
    if crown_profile_model == 'beta':
        # Species group 
        species_group = np.array([spcd_params[s]['SPGRP'] for s in species_code]).astype(str)
        # Beta profile params
        ...
        # Trait score
        trait_score = np.array([spcd_params[s]['PURVES_TRAIT_SCORE'] for s in species_code])
